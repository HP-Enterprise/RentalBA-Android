...
import com.gjcar.annotation.EventListener;
			...
			for (Field field : fields) {
				...
				if (field.isAnnotationPresent(ContentWidget.class)) {
					...
					if (viewId == 0)// 这个是获取onclick的id
						viewId = activity.getResources().getIdentifier(field.getName(), "id",activity.getPackageName());
					if (viewId == 0)
					...
					if (null != view) {
						try {
							...
							setListener(activity, field, annotation.click(),
									Method1.Click);
