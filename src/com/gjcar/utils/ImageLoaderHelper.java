...
public class ImageLoaderHelper {
	...
	public static DisplayImageOptions initDisplayImageOptions_ActivityImage(){
		
		DisplayImageOptions options = new DisplayImageOptions
			.Builder()
			.showImageOnLoading(R.drawable.page_show)//设置图片下载期间显示的图片
			.showImageForEmptyUri(R.drawable.page_show)//设置图片uri为空或是错误的时候显示的图片
			.showImageOnFail(R.drawable.page_show)//设置图片加载或解码过程时的错误显示的图片
			.cacheInMemory(true)//设置下载的图片是否缓存在内存中
			.cacheOnDisc(true)//设置下载的图片是否缓存在sd卡中	
			.considerExifParams(true)//是否考虑JPEG图像EXIF参数(旋转，翻转)
			.imageScaleType(ImageScaleType.EXACTLY_STRETCHED)//设置图片以如何的编码方式显示
	...
	public static DisplayImageOptions initDisplayImageOptions_ActivityImage_Dialog(){
		
		DisplayImageOptions options = new DisplayImageOptions
			.Builder()
			.showImageOnLoading(R.drawable.welcome)//设置图片下载期间显示的图片
			.showImageForEmptyUri(R.drawable.welcome)//设置图片uri为空或是错误的时候显示的图片
			.showImageOnFail(R.drawable.welcome)//设置图片加载或解码过程时的错误显示的图片
			.cacheInMemory(true)//设置下载的图片是否缓存在内存中
			.cacheOnDisc(true)//设置下载的图片是否缓存在sd卡中	
			.considerExifParams(true)//是否考虑JPEG图像EXIF参数(旋转，翻转)
			.imageScaleType(ImageScaleType.EXACTLY_STRETCHED)//设置图片以如何的编码方式显示
