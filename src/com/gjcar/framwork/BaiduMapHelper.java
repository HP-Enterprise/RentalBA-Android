...
//				}
	 ...
	 * 绘制多边形区域
	 */
	public void DrawPolygon(List<Point> points, BaiduMap mBaiduMap){
	
		List<LatLng> pts = new ArrayList<LatLng>();  
		
		for (int i = 0; i < points.size(); i++) {
			LatLng pt = new LatLng(points.get(i).lat, points.get(i).lng); 
			pts.add(pt);
		}
	 ...
	 * 判断点是否在多边形区域内部
	 */
	public boolean isPolygon(List<Point> points, LatLng latlng){
		...
		return SpatialRelationUtil.isPolygonContainsPoint(pts, latlng);
	}
}
