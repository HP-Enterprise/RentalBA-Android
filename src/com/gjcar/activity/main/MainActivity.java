...
import com.gjcar.data.bean.AdvertisMentInfo;
...
import com.gjcar.utils.HttpHelper_Ad;
...
public class MainActivity extends FragmentActivity implements Update_Notify{
	...
	private final static int Http_Get_Advertisement = 2;
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				switch (msg.what) {
					...
					case Http_Get_Advertisement:

						if (HandlerHelper.getString(msg).equals(HandlerHelper.Ok)) {

							AdvertisMentInfo advertismentinfo = (AdvertisMentInfo)msg.obj;
							
							if (advertismentinfo.isEnable == 1 && advertismentinfo.picUrl != null){
								
								getImage(advertismentinfo);
							}else{
								
//								showShareDialog(false, null);//时间不对
							}
						}else{
							
							if (HandlerHelper.getString(msg).equals(HandlerHelper.Fail)) {
//								showShareDialog(false, null);
							}
						}
						break;	
	 ...
	 * 活动弹框
	 */
	private void showShareDialog(Bitmap bitmap,final AdvertisMentInfo advertismentinfo){
		
		final ShareImageDialog dialog = new ShareImageDialog(MainActivity.this);
		dialog.initView(bitmap);
		dialog.setCanceledOnTouchOutside(false);
//		dialog.setListener(new ShareImageDialog.ClickListener() {
//			@Override
//			public void allClick() {
//				
////				IntentHelper.startActivity_StringExtras(MainActivity.this, WebActivity.class, new String[]{"title","fragment","url"}, new String[]{"活动详情","action_detail",Public_Api.appWebSite_Ad+advertismentinfo.url});
...
//		});
	...
	private void Http_Get_Advertisement(){

		/*参数*/
		int type = 2;

		/*请求*/
		String api = "api/advertiseMentShow"+"?type="+type;
		new HttpHelper_Ad().initData(HttpHelper_Ad.Method_Get, this, api, null, null, handler, Http_Get_Advertisement, 1, new TypeReference<AdvertisMentInfo>(){});
	}

	private void getImage(final AdvertisMentInfo advertismentinfo){
		
		AsyncHttpClient httpclient = new AsyncHttpClient();  
		
        httpclient.get(Public_Api.appWebSite_Ad+advertismentinfo.picUrl, new AsyncHttpResponseHandler() {  
        	
            @Override  
            public void onSuccess(int statusCode, Header[] headers,  
                    byte[] response) {  
  System.out.println("图片大小"+response.length);
                BitmapFactory bitmapFactory = new BitmapFactory();  
                Bitmap bitmap = bitmapFactory.decodeByteArray(response, 0, response.length);  
                
                showShareDialog(bitmap,advertismentinfo);
            }  
