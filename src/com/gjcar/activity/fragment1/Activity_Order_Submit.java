...
import com.gjcar.view.widget.CustomSampleDialog;
import com.gjcar.view.widget.CustomSampleDialog.OnCenterItemClickListener;
...
public class Activity_Order_Submit extends Activity{
	...
	@ContentWidget(click = "onClick") LinearLayout discount_activity_lin;
...
//		if(Public_Param.order_paramas.activityHostType == 8){discount_lin.setVisibility(View.GONE);}
	...
	protected void onDestroy() {
		System.out.println("ggg-onDestroy");
		System.out.println("ggg-activityId-"+Public_Param.order_paramas.activityId.intValue());
		Public_Param.isUseActivity = Public_Param.order_paramas.activityId.intValue() == 0 ? false : true;
		System.out.println("ggg-"+Public_Param.isUseActivity);
		super.onDestroy();
	}
	...
	private void Request_AmountDetail(int Request_Code) {
		...
		String api = "api/searchAmountDetail?userId="+userId+"&activityId="+activityId+"&isDoorToDoor="+isDoorToDoor+"&endDate="+endDate+"&modelId="+modelId+"&startDate="+startDate+"&storeId="+takeCarStoreId+"&returnCityId="+returnCityId+"&returnStoreId="+returnStoreId+"&takeCityId="+takeCityId+"&nonDeducitible="+Public_Param.order_paramas.isServiceOk;
...
//								new OrderSubmit_Helper().initActivityDialog(this, Public_Param.order_paramas.activityShowList,handler, Click_Activity);
			...
			case R.id.ok:
				if (Public_Param.order_paramas.activityShow != null
				&& ("0".equals(Public_Param.order_paramas.activityShow.applySource) || "2"
						.equals(Public_Param.order_paramas.activityShow.applySource))
				&& Public_Param.order_paramas.activityShow.promptMessage != null) {
			CustomSampleDialog customSampleDialog = new CustomSampleDialog(
					Activity_Order_Submit.this,
					R.layout.dialog_activity_prompt,
					new int[] { R.id.activityprompt_know });
			customSampleDialog
					.setOnCenterItemClickListener(new OnCenterItemClickListener() {
						@Override
						public void OnCenterItemClick(
								CustomSampleDialog dialog, View view) {
							switch (view.getId()) {
							case R.id.activityprompt_know:
								confirmOrder();
								break;
							}
						}
					});
			customSampleDialog.show();
			TextView activityDesc = (TextView) customSampleDialog
					.findViewById(R.id.activity_prompt_desc);
			activityDesc
					.setText("\u3000\u3000"
							+ Public_Param.order_paramas.activityShow.promptMessage);
		}else {
			confirmOrder();
		}
	...
	private void confirmOrder(){
		...
		if(isRequestRealNameEmpty || isRequestNumberEmpty){
			...
			if(isRequestRealNameEmpty){
				...
				if(ValidationHelper.isNull(user_name)){ToastHelper.showToastShort(Activity_Order_Submit.this, "请填写真实姓名");return;}
			...
			if(isRequestNumberEmpty){
				...
				if(ValidationHelper.isNull(user_number)){ToastHelper.showToastShort(Activity_Order_Submit.this, "请填写真实身份证号码");return;}
				if(!(user_number.getText().toString().trim().matches("^[1-9]\\d{7}((0\\d)|(1[0-2]))(([0|1|2]\\d)|3[0-1])\\d{3}$")||user_number.getText().toString().trim().matches("^[1-9]\\d{5}[1-9]\\d{3}((0\\d)|(1[0-2]))(([0|1|2]\\d)|3[0-1])\\d{3}([0-9]|X|x)$"))){ToastHelper.showToastShort(Activity_Order_Submit.this, "身份证号码填写不正确");return;}	
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				switch (msg.what) {
					...
					case UpdateFail:
						SubmitDialog.closeSubmitDialog();
						ToastHelper.showToastShort(Activity_Order_Submit.this, msg.getData().getString("errorMsg"));//显示发送失败的原因
						break;
	
					case UpdateDataFail:
						SubmitDialog.closeSubmitDialog();
						ToastHelper.showToastShort(Activity_Order_Submit.this, "提交身份证信息失败");//显示发送失败的原因			
						break;
...
//		}
		...
		/*可选服务的费用统计*/	
		serviceAllAmount = 0;System.out.println("不计免赔不添加-第几-"+MyFlag);
...
//					}
					...
					if(Public_Param.order_paramas.isSdew == 1 && MyFlag == Use_Activity){//如果=1，就不添加
					
						System.out.println("不计免赔不添加");
					}else{
						System.out.println("不计免赔不添加--t");
	...
	private void init_Actions(int flag){
		...
		switch (flag) {
			...
			case Use_Ticket:
				...
				/*显示:用于显示的*/
				int amount_show = ticket_list.get(Ticket_List_Position).amount.intValue();
				d_activity_all.setText("￥-"+amount_show);
...
//		String consume = new Integer(orderPrice.totalPrice.intValue()+serviceAllAmount).toString();
		String consume = orderPrice.totalPrice.intValue()+"";
//		String api = "api/me/coupon?consume="+consume+"&currentPage=1&pageSize=100&state=2&uid="+uid+"&willBeUseTimeBegin="+willBeUseTimeBegin+"&willBeUseTimeEnd="+illBeUseTimeEnd+"&source=4";
		String rental = orderPrice.totalAmount.toString();
		String cityId = Public_Param.order_paramas.takeCarCityId;
		String storeId = Public_Param.order_paramas.takeCarStoreId;
		String modelId = Public_Param.order_paramas.modelId.toString();
		String useTime = TimeHelper.getNowTime_YMDHM_Tickert();
		String api = "api/me/canUsedCoupon?source=4&consume="+consume+"&currentPage=1&pageSize=100&state=2&uid="+uid+"&rental="+rental+"&cityId="+cityId+"&storeId="+storeId+"&modelId="+modelId+"&useTime="+useTime;
