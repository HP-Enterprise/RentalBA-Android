 ...
 *  	onMeasure：
 *  做法：
 *  	ExpandableListView点击事件，整体布局
 *  	而每一个Item由Adapter来显示，所以
 *  	ExpandableListView将数据参数传递到Adapter中
 *  	所以定义了一个借口，用于ExpandableListView将数据参数传递到Adapter中
 *  	*/
public class QQListView_City extends ExpandableListView implements OnScrollListener,OnGroupClickListener {
	public QQListView_City(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		registerListener();
	}

	public QQListView_City(Context context, AttributeSet attrs) {
		super(context, attrs);
		registerListener();
	}

	public QQListView_City(Context context) {
		super(context);
		registerListener();
	}
	...
	public interface QQHeaderAdapter {
		 ...
		 * @param alpha
		 */
		void configureQQHeader(View header, int groupPosition,int childPosition, int alpha);
	...
	public void configureHeaderView(int groupPosition, int childPosition) {
		...
		switch (state) {
			...
			case QQHeaderAdapter.PINNED_HEADER_VISIBLE: {
				mAdapter.configureQQHeader(mHeaderView, groupPosition,childPosition, MAX_ALPHA);
	
//				if (mHeaderView.getTop() != 0){//*****
//					mHeaderView.layout(0, 0, mHeaderViewWidth, mHeaderViewHeight);
//				}
				if (mHeaderView.getTop() != -1){//*****
					mHeaderView.layout(0, 0, mHeaderViewWidth, mHeaderViewHeight);
				}
			...
			case QQHeaderAdapter.PINNED_HEADER_PUSHED_UP: {
				...
				mAdapter.configureQQHeader(mHeaderView, groupPosition,childPosition, alpha);
	...
	@Override
	public void onScroll(AbsListView view, int firstVisibleItem,int visibleItemCount, int totalItemCount) {System.out.println("onScroll");
	...
	@Override
	public void onScrollStateChanged(AbsListView view, int scrollState) {System.out.println("onScrollStateChanged");
